...
vec4 bilerp(sampler2D sam, vec2 uv) {
    vec2 p_uv = uv * c_size;
    vec2 weights = fract(p_uv);
    vec4 a = texture2D(sam, vec2(floor(p_uv.x), (floor(p_uv.y)))/c_size); 
    //texture2D(sam, uv + vec2(-1.0, -1.0) * t_size);
    vec4 b = texture2D(sam, vec2(ceil(p_uv.x), (floor(p_uv.y)))/c_size);
    //texture2D(sam, uv + vec2(1.0, -1.0) * t_size);
    vec4 c = texture2D(sam, vec2(floor(p_uv.x), (ceil(p_uv.y)))/c_size);
    //texture2D(sam, uv + vec2(-1.0, 1.0) * t_size);
    vec4 d = texture2D(sam, vec2(ceil(p_uv.x), (ceil(p_uv.y)))/c_size); 
    //texture2D(sam, uv + vec2(1.0, 1.0) * t_size);
    return mix(mix(a, b, weights.x), mix(c, d, weights.x), weights.y);
}
